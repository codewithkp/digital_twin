...
function cellClass(val: number, thresh: { amber: number; red: number }, inv: boolean) {
  ...
  if (s === 'RED')   return 'bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300';
  if (s === 'AMBER') return 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300';
  return '';
}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <div key={label} className="bg-white dark:bg-slate-800 rounded-lg p-3 border border-slate-200 dark:border-slate-700">
            ...
            <p className="text-slate-900 dark:text-white font-semibold text-sm mt-1">{val}</p>
          </div>
        ))}
      </div>

      {Object.entries(quality.missingByCol).some(([, v]) => v > 0) && (
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-300 dark:border-amber-600/30 rounded-lg p-3 text-xs text-amber-700 dark:text-amber-300">
          <span className="font-semibold">Missing values detected: </span>
          {Object.entries(quality.missingByCol).filter(([, v]) => v > 0).map(([col, v]) => `${col}: ${v}`).join(' · ')}
        </div>
      )}
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
                ...
                {[1, 2, 3, 4].map((n) => <th key={`bt${n}`} className="px-3 py-2 text-right font-medium whitespace-nowrap">BT{n} (°C)</th>)}
                {[1, 2, 3, 4].map((n) => <th key={`vr${n}`} className="px-3 py-2 text-right font-medium whitespace-nowrap">VR{n} (mm/s)</th>)}
              </tr>
            </thead>
            <tbody>
              {preview.map((row, idx) => (
                <tr key={idx} className="border-t border-slate-200 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  <td className="px-3 py-1.5 text-slate-500 dark:text-slate-400 whitespace-nowrap font-mono">
                  ...
                  {(row.sensors.bearingTemp as number[]).map((v, i) => (
                    <td key={i} className={`px-3 py-1.5 text-right font-mono ${cellClass(v, THRESHOLDS.bearingTemp, false)}`}>{v.toFixed(1)}</td>
                  ))}
                  {(row.sensors.vibrationRMS as number[]).map((v, i) => (
                    <td key={i} className={`px-3 py-1.5 text-right font-mono ${cellClass(v, THRESHOLDS.vibrationRMS, false)}`}>{v.toFixed(2)}</td>
                  ))}
                </tr>
              ))}
            </tbody>
