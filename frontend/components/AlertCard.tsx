...
const SEV_STYLES = {
  RED:   { icon: '🔴', border: 'border-red-500/50',   bg: 'bg-red-50 dark:bg-red-900/10'     },
  AMBER: { icon: '🟡', border: 'border-amber-400/40', bg: 'bg-amber-50 dark:bg-amber-900/10' },
...
export function AlertCard({ alert, onFlag }: AlertCardProps) {
  ...
  const handleFlag = async () => {
    await fetch('/api/maintenance-flags', {
      ...
      body: JSON.stringify({ componentId: alert.groupId ?? alert.id, sensor: alert.sensor, timestamp: alert.timestamp }),
    });
  ...
  return (
    <div className={`rounded-lg border p-3 ${border} ${bg} text-sm`}>
      ...
      <div className="text-slate-600 dark:text-slate-300 text-xs mb-2">
        ...
        <span className="text-slate-400 dark:text-slate-500"> {alert.unit} </span>
        <span className="text-slate-400 dark:text-slate-500">(threshold: {alert.threshold} {alert.unit})</span>
      </div>
      <button
        onClick={handleFlag}
