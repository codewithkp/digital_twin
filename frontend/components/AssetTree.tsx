...
interface AssetTreeProps { alerts: Alert[]; }
interface TreeRowProps { label: string; status: HealthStatus; indent?: boolean; flagged?: boolean; }

function TreeRow({ label, status, indent = false, flagged }: TreeRowProps) {
  return (
    <div className={`flex items-center gap-2 py-1 ${indent ? 'pl-5' : ''}`}>
      <span className="text-slate-400 dark:text-slate-600 select-none">{indent ? '├─' : '└─'}</span>
      ...
      {flagged && <span className="text-amber-500 text-[10px] ml-auto">🔧</span>}
    </div>
  );
}
...
export function AssetTree({ alerts }: AssetTreeProps) {
  ...
  const overallStatus: HealthStatus =
    ...
    alerts.some((a) => a.severity === 'AMBER') ? 'AMBER' : 'GREEN';

  return (
    <div>
      <p className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Asset Tree</p>

      <div className="flex items-center gap-2 mb-1">
        <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${DOT[overallStatus]}`} />
        <span className="text-blue-600 dark:text-blue-400 text-sm font-semibold">Conveyor C-01</span>
      </div>

      <div className="pl-3 border-l border-slate-200 dark:border-slate-700">
        <div className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider py-1">Idler Groups</div>
        {groups.map((g) => (
          <TreeRow key={g.groupId} label={g.label} status={g.status} indent />
        ))}
      </div>

      <div className="pl-3 border-l border-slate-200 dark:border-slate-700 mt-1">
        <div className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider py-1">Drive System</div>
